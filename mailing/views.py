...
class MailingCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if new_mailing.mailing_time <= timezone.now():
            ...
            try:
                ...
                log = Log.objects.create(date_attempt=timezone.now(), status='Успешно', answer='200', mailing=new_mailing)
                log.save()
            except smtplib.SMTPDataError as err:
                log = Log.objects.create(date_attempt=timezone.now(), status='Ошибка', answer=err, mailing=new_mailing)
                log.save()
